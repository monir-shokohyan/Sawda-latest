...
const MessageBubble = styled(Paper)<PaperProps & { $isOwn: boolean, children: ReactNode }>`
  max-width: 70%;
  padding: 12px 16px;
  background: ${({ $isOwn }) =>
    $isOwn ? 'var(--mantine-color-primary-6)' : 'var(--mantine-color-gray-1)'};
  color: ${({ $isOwn }) =>
    $isOwn ? 'white' : 'var(--mantine-color-dark-9)'};
  border-radius: ${({ $isOwn }) =>
...
const ChatBubble = ({ message, username }: ChatBubbleProps) => {
  return (
    <BubbleWrapper $isOwn={message.isOwn} wrap="nowrap" align="flex-end" gap="xs">
      {!message.isOwn && (
      ...
      <Stack gap={4}>
        <MessageBubble $isOwn={message.isOwn} shadow="sm">
